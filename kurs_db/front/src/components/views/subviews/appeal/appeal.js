...
const appeal = (props) => {
    ...
    const {id_appeal, id_contract, description, jobs} = data || {}

    const onDelete = () => {
        let url = new URL(`appeals/${id}`, 'http://localhost:8080')
    ...
    const toggleEdit = async () => {
        if (status == STATUS.IDLE) {
            ...
            let {appeal} = await req.json()
            dispatch({type: "IDLE", data: appeal})
        }
        ...
        <Grid container>
            ...
            <Grid container item xs={12} lg={12} md={12} justify='center' className={classes.buttonsContainer}>
                <ButtonGroup color="primary" aria-label="outlined primary button group">
                    {RULES["appeals"]["DELETE"].includes(user.role) && <Button onClick={onDelete}>Удалить</Button>}
                    {RULES["appeals"]["PUT"].includes(user.role) && <Button onClick={toggleEdit}>{status == STATUS.EDIT ? "Сохранить" : "Редактировать"}</Button>}
                    {RULES["appeals"]["PUT"].includes(user.role) &&<Button onClick={handleOpen}>История</Button>}
                </ButtonGroup>
            </Grid>
            ...
            <Grid container justify='center' className={classes.table}> 
                {data && RULES["jobs"]["GET"].includes(user.role) &&
                    <Jobs
                        appeal={data}
                    />
                }
