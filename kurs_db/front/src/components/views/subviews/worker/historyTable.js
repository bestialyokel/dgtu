...
const HistoryTable = (props) => {
    const {worker, onSelect} = props
    const {id_worker} = worker
    const token = useContext(TokenContext)

    const [state, setState] = useState({columns, data: []})

    const fillData = async () => {
        let url = new URL(`workers/history/${id_worker}`, 'http://localhost:8080')
    ...
    return (
        <Table
            title={`История изменений рабочего: ${[worker.name, worker.surname, worker.patronymic].join(' ')}`}
