...
const service = (props) => {
    ...
    const {id_service, name, description, /*status */} = data || {}


    const onDelete = () => {
        let url = new URL(`services/${id}`, 'http://localhost:8080')
    ...
    const toggleEdit = async () => {
        if (status == STATUS.IDLE) {
            ...
            dispatch({type: "IDLE", data: service})
        }
        ...
        <Grid container>
            ...
            <Grid container item xs={12} lg={12} md={12} justify='center' className={classes.buttonsContainer}>
                <ButtonGroup color="primary" aria-label="outlined primary button group">
                    { RULES["services"]["DELETE"].includes(user.role) && <Button onClick={onDelete}>Удалить</Button>}
                    { RULES["services"]["PUT"].includes(user.role) && <Button onClick={toggleEdit}>{status == STATUS.EDIT ? "Сохранить" : "Редактировать"}</Button>}
                    { RULES["services"]["PUT"].includes(user.role) && <Button onClick={handleOpen}>История</Button>}
                </ButtonGroup>
            </Grid>
            <Grid container spacing={6} className={classes.container}>
                <Grid container item xs={12} md={6} lg={3} justify='center'>
                    <TextField disabled id="standard-required" label="ID Услуги" defaultValue={id_service}/>
                </Grid>
                <Grid container item xs={12} md={6} lg={3} justify='center'>                            {/* какого хуя везде адекватно работает defaultValue
                    ...
                    <TextField disabled={status == STATUS.IDLE} id="standard-required" label="Описание" value={name} 
